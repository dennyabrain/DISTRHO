...
public:
    ...
    void processReplacing (float** inputs, float** outputs, VstInt32 numSamples)
    {
        ...
        if (PorPR!=1) {
            ...
            if (getCurrentPosition(pos)) {
                if (pos.isPlaying && !lastPos.isPlaying) logDebugString("playing");
                else if (!pos.isPlaying && lastPos.isPlaying) logDebugString("stopped");
                if (pos.isRecording && !lastPos.isRecording) logDebugString("recording");
                else if (!pos.isRecording && lastPos.isRecording) logDebugString("not recording");
    ...
    {
#ifdef MAKE_LOG
        if (onlyStoreCurrentProgramData) OutputDebugString("getChunk(preset)");
        else OutputDebugString("getChunk(bank)");
#endif
        if (filter == 0)
            return 0;
#if 0
        ...
        if (String(host).equalsIgnoreCase(String("REAPER"))) {
            onlyStoreCurrentProgramData=false;
        }         
#endif
    ...
    {
#ifdef MAKE_LOG
        if (onlyRestoreCurrentProgramData) OutputDebugString("setChunk(preset)");
        else OutputDebugString("setChunk(bank)");
#endif
        if (filter == 0)
            return 0;
#if 0
        ...
        getHostProductString (host);
        if (String(host).equalsIgnoreCase(String("REAPER"))) {
            onlyRestoreCurrentProgramData=false;
        }
#endif
        chunkMemory.setSize (0);
        chunkMemoryTime = 0;
...
String getHostPath( )
{
   char* outInstancePath = new char[256];
   WCHAR filename[256];
   ...
   for ( int i = countloc; i > 0; i-- )
   {
     ...
     //OutputDebugString(outInstancePath);
...
//PIZ=================================================
    ...
    //OutputDebugString((const char*)currentFileName);
#ifdef PIZMIDI
#define PIZ_FX_MAGIC 1
    String hostname = getHostVendor();
    if (!hostname.isEmpty()) {
        if (hostname.contains(T("Ableton"))) {
            inst=true;
            numOutChans=2;
        }
        else if (hostname.contains(T("Steinberg"))) {
            inst=true;
            numOutChans=2;
        }
        else if (hostname.contains(T("Twelve Tone"))) {
            inst=true;
            numOutChans=2;
        }
        else if (hostname.contains(T("Image-Line"))) {
            inst=true;
            numOutChans=2;
        }
        else if (hostname.contains(T("Polac"))) {
            inst=true;
            numOutChans=0;
        }
        else if (hostname.contains(T("Native Instruments"))) {
            inst=false;
			numInChans=2;
            numOutChans=2;
        }
    ...
    //read "pizmidi.ini" in host's directory
    String pizmidiIniPath = getHostPath() + File::separatorString + T("pizmidi.ini");
    if (File(pizmidiIniPath).exists()) {
        ...
        for (int i=0;i<lines.size();i++) {
            if (!lines[i].startsWithChar(';')) {
                if (lines[i].contains(T("host=") + hostname)) {
                    hostmatch=true;
                }
                else if (lines[i].contains(T("host="))) {
                    hostmatch=false;
                }
                if (hostmatch) {
                    if      (lines[i].equalsIgnoreCase(T("force inst")))    inst=true;
                    else if (lines[i].equalsIgnoreCase(T("force effect")))  inst=false;
                    else if (lines[i].equalsIgnoreCase(T("audio outs")))    numOutChans = 2;
                    else if (lines[i].equalsIgnoreCase(T("no audio outs"))) numOutChans = 0;
                    else if (lines[i].equalsIgnoreCase(T("audio ins")))     numInChans = 2;
                    else if (lines[i].equalsIgnoreCase(T("no audio ins")))  numInChans = 0;
                }
            }
        }
    }
...
#if PIZ_FX_MAGIC
    if (currentFileName.containsIgnoreCase(T("nofx"))) {
        //force instrument only
        numInChans = 0;
        inst = true;
        VSTID = (int)(Piz_InstID);
    }
    else if (currentFileName.containsIgnoreCase(T("fx"))) {
        //force effect only
        numInChans = 2;
        inst = false;
...
#ifndef PIZ_NO_GUI
    if (!currentFileName.containsIgnoreCase(T("noGUI"))) {
        cEffect.flags |= effFlagsHasEditor;
    }
#endif
//====================================================
...
//====================================================
    ...
    char text[256];
    dbg = getHostVendorString(text)? ("getHostVendorString(): "+String(text)) : "getHostVendorString(): (false)";
    logDebugString(dbg);
