...
void MidiAutomatable::handleMidiPopupMenu_callback(int result)
{
    switch (result)
    {
    case 1:
...
void MidiAutomatable::handleMidiPopupMenu_staticCallback (int result, MidiAutomatable* widget)
{
    if (widget != 0)
        widget->handleMidiPopupMenu_callback (result);
}
...
void MidiAutomatable::handleMidiPopupMenu (const MouseEvent& e)
{
    ...
    // TODO-XXX
    menu.showMenuAsync (PopupMenu::Options().withTargetComponent (e.eventComponent),
                        ModalCallbackFunction::create (handleMidiPopupMenu_staticCallback, this));

    //menu.showAt (e.getScreenX(), e.getScreenY()); //FIXME-XXX
}
