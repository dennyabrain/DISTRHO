    ...
    */
    bool restoreProgramFromFile (const File& file, const bool addProgram = true)
    {
        ...
        if (xml == 0 || ! xml->hasTagName ("preset"))
        {
            ...
            printf ("Error parsing preset XML: %s\n", (const char*) errString.toUTF8());
        }
        else
        {
#if 0
            int pluginUniqueID = xml->getIntAttribute (T("uniqueid"), 0);
            if (owner->getFilterID () == pluginUniqueID)
#endif
            {
                // current preset
                XmlElement* chunk = xml->getChildByName (T("chunk"));
                if (chunk)
                {
                    // presets are chunk based
                    forEachXmlChildElement (*chunk, dataElement)
                    {
                        if (dataElement->hasTagName (T("data")))
                        {
                            MemoryBlock mb;
                        ...
                        if (dataElement->hasTagName (T("params")))
                        {
    ...
    */
    bool storeProgramToFile (const File& file, const bool addProgram = true)
    {
        ...
        if (mb.getSize () > 0)
        {
            ...
            xml.setAttribute (T("name"), getName ());
            xml.setAttribute (T("author"), T(""));
            xml.setAttribute (T("category"), T(""));
            xml.setAttribute (T("comments"), T(""));

            XmlElement* chunk = new XmlElement (T("chunk"));
            {
                XmlElement* dataElement = new XmlElement (T("data"));
                dataElement->addTextElement (mb.toBase64Encoding ());
                chunk->addChildElement (dataElement);

                XmlElement* paramElement = new XmlElement (T("params"));
...
#endif // __JUCETICE_AUDIOPLUGIN_HEADER__
