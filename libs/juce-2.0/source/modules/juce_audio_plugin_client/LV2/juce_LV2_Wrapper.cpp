 ...
 * - JucePlugin_WantsLV2TimeBPM
 * - JucePlugin_WantsLV2InstanceAccess (UI)
 */

#if JucePlugin_WantsLV2TimePos
#error NOT IMPLEMENTED YET
#ifdef JucePlugin_WantsLV2TimeBPM
#undef JucePlugin_WantsLV2TimeBPM
#endif
#endif
...
String makePluginTtl(AudioProcessor* const filter, const String& binary)
{
    ...
    if (filter->hasEditor())
    {
        plugin += "    ui:ui <" JucePlugin_LV2URI "#X11UI> ,\n";
        plugin += "          <" JucePlugin_LV2URI "#ExternalUI> ;\n";
        plugin += "\n";
    }
...
private:
    ...
    void resetExternalUI(const LV2_Feature* const* features)
    {
        ...
        for (uint16 i = 0; features[i]; i++)
        {
            if (strcmp(features[i]->URI, LV2_EXTERNAL_UI_URI) == 0)
            {
...
#if JucePlugin_WantsLV2TimePos
#elif JucePlugin_WantsLV2TimeBPM
        if (portId == index)
        {
...
void juceLV2_Save(LV2_Handle instance, LV2_State_Store_Function store, LV2_State_Handle handle, uint32_t flags, const LV2_Feature* const* features)
{
    ...
    store (handle,
           uridMap->map(uridMap->handle, "urn:juce:stateString"),
