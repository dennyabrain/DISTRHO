...
namespace madlibNamespace
{
#if JUCETICE_INCLUDE_MADLIB_CODE
#if JUCE_MAC
// haszari: inhibit namespaceless assert for madlib on mac, some problem with inclusion    
#define assert 
#endif
#define FPM_INTEL 1
  #define FPM_64BIT 1
...
//  #define HAVE_STRING_H 1
  ...
  extern "C" {
    #include "../../dependancies/mad/bit.c"
    #include "../../dependancies/mad/decoder.c"
    #include "../../dependancies/mad/fixed.c"
    #include "../../dependancies/mad/frame.c"
    #include "../../dependancies/mad/huffman.c"
    #include "../../dependancies/mad/layer12.c"
    #include "../../dependancies/mad/layer3.c"
    #include "../../dependancies/mad/stream.c"
    #include "../../dependancies/mad/synth.c"
    #include "../../dependancies/mad/timer.c"
    #include "../../dependancies/mad/version.c"
  }
#else
  #include <mad.h>
#endif
...
using namespace madlibNamespace;
...
public:
	...
	MADAudioFormatReader(InputStream* const sourceStream) :
		AudioFormatReader(sourceStream, "MPEG Audio Decoder (MAD)")
	{
...
//==============================================================================
static const char* const mad_extensions[] = { ".mp3", ".mp2", ".mpg", 0 };

MADAudioFormat::MADAudioFormat()
    : AudioFormat("MPEG Audio Decoder (MAD)", StringArray(mad_extensions))
{
}
...
{
	return fileToTest.hasFileExtension("mp3") ||
			fileToTest.hasFileExtension("mp2") ||
			fileToTest.hasFileExtension("mpg");
}

Array<int> MADAudioFormat::getPossibleSampleRates()
{
    const int rates[] = { 44100, 48000, 0 };
    return Array <int>(rates);
}

Array<int> MADAudioFormat::getPossibleBitDepths()
{
...
//==============================================================================
AudioFormatReader* MADAudioFormat::createReaderFor(InputStream* sourceStream, bool deleteStreamIfOpeningFails)
{
