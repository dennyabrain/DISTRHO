...
struct MidiEvent {
    ...
    MidiEvent()
        ...
        : frame(0),
          buffer{0} {}
#else
        : frame(0) { memset(buffer, 0, sizeof(uint8_t)*3); }
#endif
};
...
protected:
    ...
    virtual void d_run(const float* const* inputs, float* const* outputs, uint32_t frames, uint32_t midiEventCount, const MidiEvent* midiEvents) = 0;

    // Callbacks
    virtual void d_bufferSizeChanged(uint32_t newBufferSize) = 0;
