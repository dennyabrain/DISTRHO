    ...
    : AudioEffectX(audioMaster, 1, kNumParams)	// 1 program, 1 parameter only
{
    ...
    setUniqueID((long)"Bits");	// identify
...
//------------------------------------------------------------------------
bool Bitcrusher::getEffectName (char* name)
{
    strcpy (name, "Bitcrusher");
    return true;
}

//------------------------------------------------------------------------
bool Bitcrusher::getProductString (char* text)
{
    strcpy (text, "Bitcrusher");
    return true;
}

//------------------------------------------------------------------------
bool Bitcrusher::getVendorString (char* text)
{
    strcpy (text, "CMT");
    return true;
}
