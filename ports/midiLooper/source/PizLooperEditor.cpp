...
void PizLooperEditor::changeListenerCallback (ChangeBroadcaster* source)
{
	...
	{
		loopinfoLabel2->setText(getFilter()->info->s,sendNotificationSync);
	}
...
void PizLooperEditor::updateControls(int param, float value, bool forCurProgram)
{
	...
	case kMonitor:
		...
		break;
	case kSync:
		syncmodeBox->setText(getFilter()->getCurrentSlotParameterText(kSync),sendNotificationSync);
		break;
	case kRecStep:
		stepsizeBox->setText(getFilter()->getCurrentSlotParameterText(kRecStep),sendNotificationSync);
		break;
	case kQuantize:
		quantizeBox->setText(getFilter()->getCurrentSlotParameterText(kQuantize),sendNotificationSync);
		break;
	case kFixedLength:
	...
	case kStretch:
		...
		break;
	case kTrigger:
		loopmodeBox->setText(getFilter()->getCurrentSlotParameterText(kTrigger),sendNotificationSync);
		break;
	case kNoteTrig:
		notetriggerBox->setText(getFilter()->getCurrentSlotParameterText(kNoteTrig),sendNotificationSync);
		break;
	case kRoot:
	...
	case kForceToScaleMode:
		forceModeBox->setText(getFilter()->getCurrentSlotParameterText(kForceToScaleMode),sendNotificationSync);
		break;
	case kTranspose10:
...
void PizLooperEditor::updateParametersFromFilter()
{
    ...
    // ..and after releasing the lock, we're free to do the time-consuming UI stuff..
	{
		...
		if	   (q==0.0) quantizeBox2->setText("4th",sendNotificationSync);
		else if (q<0.3) quantizeBox2->setText("8th",sendNotificationSync);
		else if (q<0.6) quantizeBox2->setText("16th",sendNotificationSync);
		else if (q<0.9) quantizeBox2->setText("32nd",sendNotificationSync);
		else quantizeBox2->setText("64th",sendNotificationSync);
