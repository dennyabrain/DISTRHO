...
package.config["Release"].buildoptions = { "-O2 -ffast-math -fomit-frame-pointer -fopenmp -funroll-loops -fvisibility=hidden -mtune=generic -msse -mfpmath=sse" }
...
if (windows) then
  ...
  package.linkoptions  = { package.linkoptions, "-static -mwindows" }
elseif (macosx) then
  package.defines      = { package.defines, "MAC=1" }
  package.buildoptions = { package.buildoptions, "-ObjC++" }
  package.linkoptions  = { package.linkoptions, "-framework AudioToolbox -framework Cocoa -framework CoreAudio -framework CoreMidi -framework QuartzCore" }
else
  package.defines      = { package.defines, "LINUX=1" }
  package.buildoptions = { package.buildoptions, "-std=c++0x" }
  package.links        = { "pthread", "asound", "dl", "rt", "X11", "Xext", "gomp" }
