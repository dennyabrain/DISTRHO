...
void DemoJuceFilter::getStateInformation (MemoryBlock& destData)
{
    ...
    xmlState.setAttribute (T("delayTime"), delayTime);
	xmlState.setAttribute (T("feedback"), feedback);
...
void DemoJuceFilter::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    if (xmlState != 0)
    {
        ...
        if (xmlState->hasTagName (T("MYPLUGINSETTINGS")))
        {
            ...
            delayTime = (float) xmlState->getDoubleAttribute (T("delayTime"), delayTime);
			feedback = (float) xmlState->getDoubleAttribute (T("feedback"), feedback);
